<?php
/**
 * Template Name: Gallery Landscape
 */
get_header();
...
<div <?php echo page_layout($post->ID); ?>>
    <?php while ( have_posts() ) : the_post();
	 ...
	 ?>
    <div class="clear"></div>
    <?php  $lnadscape_gallery_width=get_option('lnadscape_gallery_width');
			$lnadscape_gallery_height=get_option('lnadscape_gallery_height');
			$image_title=get_option('image_title');
			$lnadscape_gallery_width=$lnadscape_gallery_width?$lnadscape_gallery_width:'225';
			$lnadscape_gallery_height=$lnadscape_gallery_height?$lnadscape_gallery_height:'150';
	...
	$i = 0; //defined by WP
	$nums= get_option('gallery_images_limits')? get_option('gallery_images_limits'): '10';	
	...
	if($timthumb!="true"){ ?>
        ...
        <a href="<?php echo $imgurl; ?>" class="example2"> <img src="<?php echo get_template_directory_uri(); ?>/timthumb.php?src=<?php echo $imgurl; ?>
										&amp;w=<?php echo $lnadscape_gallery_width; ?>&amp;h=<?php echo $lnadscape_gallery_height; ?>&amp;zc=1" alt="<?php echo $title; ?>"  class="img_radius" /> </a>
        <?php
										}else{
										 $imgurl=wp_get_attachment_url($id);
										 $image = vt_resize( $id, '', $lnadscape_gallery_width, $lnadscape_gallery_height, true );  ?>
	...
	$output= "<div class='pagination'>";
	  ...
	  for($j=1; $j< count($chunked)+1; $j++){ 
		 $k = $j - 1;
		//$l = $j + 1; 
		//$output=str_replace(get_permalink($post->ID));
    ...
    <?php endwhile; // end of the loop. ?>
        ...
        <div class="clear"></div>
        <?php } ?>			
         </div><!--End mid-column-extra-width Section -->
    </div>
    <!--End content Section -->
</div>
