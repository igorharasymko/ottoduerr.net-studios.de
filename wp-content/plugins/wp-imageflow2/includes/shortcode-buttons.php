...
<div id="wpif2_sc_form" style="display:none;">
  <div id="wpif2_sc_wrap">
	<div class="wpif2-sc-header">
		<h2 class="popup-header"><?php _e( 'WP Flow Plus Shortcode Embed', 'wp-flow-plus' ); ?></h2>
	</div>

	<form id="wpif2_sc_form_element">
		<div class="wpif2-select">
			<label for="wpif2_sc_source"><?php _e( 'Where are the images coming from?', 'wp-flow-plus' ); ?></label>

			<select id="wpif2_sc_source">
					<option value="attached" SELECTED><?php _e( 'Attachments to a page/post', 'wp-flow-plus' ); ?></option>
					<option value="featured"><?php _e( 'Featured posts', 'wp-flow-plus' ); ?></option>
					<option value="folder"><?php _e( 'A subfolder', 'wp-flow-plus' ); ?></option>
					<option value="medialib"><?php _e( 'Media Library', 'wp-flow-plus' ); ?></option>
					<option value="mediatag"><?php _e( 'Media Tags collection', 'wp-flow-plus' ); ?></option>
					<option value="nextgen"><?php _e( 'NextGen Gallery', 'wp-flow-plus' ); ?></option>
				</select>
			</div>

			<div class="wpif2_sc_section wpif2_attached">
				<div>
					<label for="wpif2_sc_id"><?php _e('Page/Post ID', 'wp-flow-plus'); ?></label>
					<input type="text" name="wpif2_sc_id" value="" id="wpif2_sc_id" size="8" />
					<span class="wpif2_sc_caption"><?php _e('Default: The images attached to the current page/post', 'wp-flow-plus'); ?></span>
				</div>
			</div>

			<div class="wpif2_sc_section wpif2_medialib hide">
				<div>
					<span class="wpif2_sc_caption"><?php _e('Select the images to include below', 'wp-flow-plus'); ?></span>
				</div>
			</div>
			
			<div class="wpif2_sc_section wpif2_mediatags hide">
				<div>
					<label for="wpif2_sc_mediatag"><?php _e('Media Tag Slug', 'wp-flow-plus'); ?></label>
<?php
...
';
					...
					echo '<p>' . __('The <em>Media Tags</em> plugin is required for this option', 'wp-flow-plus') . '</p>';
					echo "</div>";
					}
?>
				</div>
			...
			<div class="wpif2_sc_section wpif2_featured hide">
				<div>
					<label for="wpif2_sc_featured"><?php _e('Select a category', 'wp-flow-plus'); ?></label>
...
?>
			...
			<div class="wpif2_sc_section wpif2_folder hide">
				<div>
					<label for="wpif2_sc_folder"><?php _e('Select a folder', 'wp-flow-plus'); ?></label>
...
';
							...
							echo __('Gallery path doesn\'t exist', 'wp-flow-plus');
						}					
?>
					</select>
					<span class="wpif2_sc_caption"><?php _e('Configure the gallery path on the', 'wp-flow-plus'); ?> <a href="<?php echo admin_url('options-general.php?page=wpFlowPlus'); ?>"><?php _e('WP Flow Plus settings page', 'wp-flow-plus'); ?></a></span>
				</div>
			</div>
...
?>
					<label for="wpif2_sc_ngg_id"><?php _e('Select a gallery', 'wp-flow-plus'); ?></label>
...
';
					...
					echo '<div class="updated inline">';
					echo '<p>' . __('The <em>NextGen Gallery</em> plugin is required for this option', 'wp-flow-plus') . '</p>';
					echo '</div>';
				  }
?>
				</div>
			...
			<h3><?php _e('Optionally enter values below to override the default settings', 'wp-flow-plus'); ?></h3>

			<div class="wpif2_sc_section wpif2_library">
				<h4><?php _e('Media Library Image Options', 'wp-flow-plus'); ?></h4>
				<div>
					<label for="wpif2_sc_order"><?php _e('Order', 'wp-flow-plus'); ?></label>
				...
				<div>
					<label for="wpif2_sc_orderby"><?php _e('Order By', 'wp-flow-plus'); ?></label>
				...
				<div>
					<label for="wpif2_sc_include"><?php _e('Include', 'wp-flow-plus'); ?>:</label>
					<input type="text" name="wpif2_sc_include" value="" id="wpif2_sc_include" class="media-input" /><button class="media-button"><?php _e('Select images to include', 'wp-flow-plus'); ?></button>
					<span class="wpif2_sc_caption"><?php _e('Comma separated list of attachment IDs', 'wp-flow-plus'); ?></span>
				</div>
				<div>
					<label for="wpif2_sc_exclude"><?php _e('Exclude', 'wp-flow-plus'); ?>:</label>
					<input type="text" name="wpif2_sc_exclude" value="" id="wpif2_sc_exclude" class="media-input" /><button class="media-button"><?php _e('Select images to exclude', 'wp-flow-plus'); ?></button>
					<span class="wpif2_sc_caption"><?php _e('Comma separated list of attachment IDs', 'wp-flow-plus'); ?></span>
				</div>
				<div>
					<label for="wpif2_sc_samewindow"><?php _e('Where should image links open', 'wp-flow-plus'); ?>?</label>
					<select id="wpif2_sc_samewindow" />
						<option value="" SELECTED></option>
						<option value="true"><?php _e('Same Window', 'wp-flow-plus'); ?></option>
						<option value="false"><?php _e('New Window', 'wp-flow-plus'); ?></option>
					</select>
				</div>
			</div>

			<div class="wpif2_sc_section wpif2_general">
				<h4><?php _e('General Options', 'wp-flow-plus'); ?></h4>
				<div>
					<label for="wpif2_sc_startimg"><?php _e('Starting Image', 'wp-flow-plus'); ?></label>
					<input type="number" name="wpif2_sc_startimg" value="" id="wpif2_sc_startimg" />
				</div>
				<div>
					<label for="wpif2_sc_rotate"><?php _e('Automatic Rotation', 'wp-flow-plus'); ?></label>
					<select id="wpif2_sc_rotate" />
						<option value="" SELECTED></option>
						<option value="on"><?php _e('On', 'wp-flow-plus'); ?></option>
						<option value="off"><?php _e('Off', 'wp-flow-plus'); ?></option>
					</select>
				</div>
				<div>
					<label for="wpif2_sc_carousel"><?php _e('Carousel Style', 'wp-flow-plus'); ?></label>
					<select id="wpif2_sc_carousel" />
						<option value="baseline" SELECTED></option>
						<option value="midline" <?php echo $disabled; ?>><?php _e('Top Aligned', 'wp-flow-plus'); ?></option>
						<option value="topline"<?php echo $disabled; ?>><?php _e('Vertical Centered', 'wp-flow-plus'); ?></option>
						<option value="angled"<?php echo $disabled; ?>><?php _e('Angled', 'wp-flow-plus'); ?></option>
						<option value="flip"<?php echo $disabled; ?>><?php _e('Flipbook', 'wp-flow-plus'); ?></option>
						<option value="explode"<?php echo $disabled; ?>><?php _e('Exploding', 'wp-flow-plus'); ?></option>
					</select>
				</div>
				<div>
					<label for="wpif2_sc_nocaptions"><?php _e('Hide Captions', 'wp-flow-plus'); ?></label>
					<select id="wpif2_sc_nocaptions" />
						<option value="" SELECTED></option>
						<option value="true"><?php _e('True', 'wp-flow-plus'); ?></option>
						<option value="false"><?php _e('False', 'wp-flow-plus'); ?></option>
					</select>
				</div>
					<label for="wpif2_sc_caption_style"><?php _e('Caption Style', 'wp-flow-plus'); ?></label>
					<select id="wpif2_sc_caption_style" />
						<option value="" SELECTED></option>
						<option value="slide-up"<?php echo $disabled; ?>><?php _e('Slide-Up', 'wp-flow-plus'); ?></option>
					</select>
				</div>
			</div>

			<div>	
				<a href="#" class="button-primary" id="wpif2_sc_insert"><?php _e('Insert', 'wp-flow-plus'); ?></a>
			</div>
	</form>
  </div>
</div>
